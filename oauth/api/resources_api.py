...
from oauth.repositories.resources import ResourceRepository
...
def create_role(app_id):
    try:
        role_repository = ResourceRepository()
...
def update_resource(app_id, resource_id):
    try:
        ...
        with session_scope() as session:
            data = request.get_json()
            role = role_repository.update(session, resource_id, app_id, **data)
            return Response(response=json.dumps(role),
...
def delete_role(app_id, resource_id):
    try:
        role_repository = ResourceRepository()
        with session_scope() as session:
            role_repository.delete(session, resource_id, app_id)
